...
namespace
{
	...
	inline std::tuple<std::vector<int>, std::vector<std::vector<int>>, std::vector<cv::Mat>> extractFeatures(const std::map<int, std::vector<cv::Mat>>& dataset, std::function<std::pair<std::vector<int>, cv::Mat>(const cv::Mat&)> extractor)
	{
...
int main(const int argc, const char **argv)
{
	if (argc != 9)
	{
		...
		std::cout << "Example: --dataset ..\\..\\..\\testData\\GeorgiaTechDatabaseCropped --extension .jpg --testNumber 2 --debug 1" << std::endl;
